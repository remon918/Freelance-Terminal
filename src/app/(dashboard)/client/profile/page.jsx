...
const ClientProfilePage = () => {
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      ...
      try {
        ...
        const profileRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/clients/${session.user.id}`);
        ...
        const paymentRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/payment-history?email=${session.user.email}`);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    // একটি লোডিং টোস্ট স্টার্ট হবে (আগের মতো টেক্সট)
    const toastId = toast.loading("Updating profile...");

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/clients/${session.user.id}`, {
        method: "PUT",
      ...
      if (response.ok && result.success) {
        ...
        // সাকসেস টোস্ট (আগের মতো টেক্সট)
        toast.success("Profile updated successfully!", { id: toastId });
      } else {
        // এরর টোস্ট (আগের মতো টেক্সট)
        toast.error(result.message || "Failed to update profile", { id: toastId });
      }
    } catch (error) {
      console.error("Update error:", error);
      // নেটওয়ার্ক এরর টোস্ট (আগের মতো টেক্সট)
  ...
  if (authLoading || loading) {
    return (
      <div className="flex flex-col h-[70vh] w-full items-center justify-center gap-2 text-inherit opacity-60 font-sans">
        ...
        <p className="text-xs font-black tracking-widest uppercase">Syncing Core Profiles...</p>
      </div>
    );
  }
  ...
  if (!session?.user || session?.user?.role !== "client") {
    return (
      <div className="text-center mt-20 text-rose-500 font-sans p-6 border border-rose-500/10 bg-rose-500/5 rounded-2xl max-w-md mx-auto shadow-sm">
        ...
        <p className="text-sm font-black uppercase tracking-wider">Access Denied</p>
        <p className="text-xs opacity-60 mt-1 font-medium">Please authenticate using an authorized Client account.</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-6 mt-14 md:mt-0 font-sans text-inherit space-y-8 antialiased selection:bg-cyan-500/20 selection:text-cyan-500 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-current/10 pb-5">
        <div className="space-y-1">
          <h1 className="text-2xl md:text-3xl font-black tracking-tight flex items-center gap-2">
            <Layers className="w-6 h-6 text-cyan-500 stroke-[2.5]" /> Partner Workspace
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="border border-current/10 bg-current/5 rounded-3xl p-6 space-y-6 relative overflow-hidden backdrop-blur-md">
          ...
          <div className="flex flex-col items-center text-center space-y-3 pt-4">
            <div className="relative p-1 bg-linear-to-tr from-cyan-400 to-teal-400 rounded-full shadow-[0_4px_12px_rgba(6,182,212,0.15)]">
              <div className="w-24 h-24 rounded-full overflow-hidden bg-zinc-900 flex items-center justify-center text-3xl font-black text-white">
                {clientInfo?.image?.startsWith("http") ? (
                  ...
                  <img src={clientInfo.image} alt={clientInfo.name} className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="space-y-1">
              <h2 className="text-lg font-extrabold tracking-tight">{clientInfo?.name}</h2>
          ...
          <div className="border-t border-current/10 pt-4 space-y-3 text-xs font-bold opacity-75">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="truncate opacity-90 max-w-40 font-mono">{clientInfo?.email}</span>
            </div>
            <div className="flex items-center justify-between gap-3">
              <span className="flex items-center gap-2">
                <Calendar className="w-3.5 h-3.5 text-cyan-500" /> System Onboard
              </span>
              <span className="opacity-90">
                {clientInfo?.createdAt ? new Date(clientInfo.createdAt).toLocaleDateString(undefined, {year: 'numeric', month: 'short', day: 'numeric'}) : "N/A"}
              </span>
            </div>
          </div>
          ...
          {isEditing ? (
            <div className="border border-current/10 bg-current/5 rounded-3xl p-6 md:p-8 backdrop-blur-md space-y-6 animate-in fade-in duration-200">
              <h3 className="text-base font-extrabold  flex items-center gap-2 uppercase tracking-wider text-cyan-400">
                <Edit3 className="w-4 h-4 stroke-[2.5]" /> Update Identification Profile
              </h3>
              
              <form onSubmit={handleUpdateProfile} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-black uppercase tracking-widest opacity-50">Authorized Name</label>
                    <input
                      type="text"
                      required
                      value={name}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-black uppercase tracking-widest opacity-50">Profile Image URL</label>
                    <input
                      type="text"
                      value={image}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase tracking-widest opacity-50">Corporate Bio / Description</label>
                  <textarea
                    rows={4}
                    value={bio}
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  >
                    {isUpdating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Globe className="w-3.5 h-3.5 stroke-[2.5]" />} 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="border border-current/10 bg-linear-to-br from-current/5 to-transparent rounded-3xl p-6 flex items-center justify-between relative overflow-hidden group">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] opacity-50 font-black uppercase tracking-widest">Total Capital Invested</p>
                    <p className="text-3xl font-black text-cyan-400 tracking-tight mt-0.5">${totalSpend}</p>
                  </div>
                </div>
              ...
              <div className="border border-current/10 bg-linear-to-br from-current/5 to-transparent rounded-3xl p-6 flex items-center justify-between relative overflow-hidden group">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] opacity-50 font-black uppercase tracking-widest">Hired Tasks Deployments</p>
                    <p className="text-3xl font-black tracking-tight mt-0.5">{paymentHistory.length}</p>
                  </div>
                </div>
      ...
      <div className="border border-current/10 bg-current/5 rounded-3xl p-6 backdrop-blur-md space-y-4">
        <h3 className="text-base font-extrabold flex items-center gap-2 uppercase tracking-wider opacity-90">
          <History className="w-5 h-5 text-cyan-500 stroke-2" /> Funding & Allocation History
        </h3>
        
        {paymentHistory.length === 0 ? (
          <div className="border border-dashed border-current/10 rounded-2xl p-8 text-center">
            <p className="text-xs opacity-40 font-bold italic">No payment records found.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-xs font-bold">
              ...
              <tbody className="divide-y divide-current/5">
                {paymentHistory.map((item) => (
                  <tr key={item._id} className="opacity-85 hover:opacity-100 hover:bg-current/5 transition-all">
                    <td className="py-4 px-4 max-w-50 sm:max-w-xs truncate font-extrabold text-inherit" title={item.taskTitle}>
                      {item.taskTitle}
                    </td>
                    <td className="py-4 px-4 font-mono opacity-50 max-w-30 truncate" title={item.sessionId}>
                      {item.sessionId}
                    </td>
                    <td className="py-4 px-4 opacity-60 font-medium">
                      {item.createdAt ? new Date(item.createdAt).toLocaleDateString(undefined, {year: 'numeric', month: 'short', day: 'numeric'}) : "N/A"}
                    </td>
